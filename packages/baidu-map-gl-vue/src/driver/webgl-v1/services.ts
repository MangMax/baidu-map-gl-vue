 ...
 *
 * JSONP 错误嗅探（`captureJsonpServiceError`）在 M3A2-SERVICES-NATIVE（#23）抽到
 * `../normalize/jsonpProbe`——v4 Service Facet 需要同一套「空结果 vs 失败」判定，
 * 而嗅探逻辑只依赖 `rawSdk._rd` 的形状、与 engine 无关。这里保留同名再导出，
 * 既有 import 路径（`./services`）与测试不变。
 */
...
export { captureJsonpServiceError, type JsonpErrorCapture } from "../normalize/jsonpProbe";
